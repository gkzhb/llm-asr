...
> 历史规划快照：本文件最初交付时尚未下载权重或开展构建/实测。后续已实现 Nix 工具链、模型转换与原生手机转写；**当前状态以 [P0 执行记录](../reports/p0/status.md) 和 [工作日志](../progress.md) 为准**。本文的预算与目标仍不是实测成绩；APK/IME 尚未实现。
>
> 目标设备：iQOO Z1 / 天玑 1000+ / 8GB RAM。优先本地离线运行，不要求 root，不默认上传录音。
